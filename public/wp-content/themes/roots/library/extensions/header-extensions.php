...
function thematic_head_scripts() {
    ...
    $scripts .= $scriptdir_start . 'application.js' . $scriptdir_end . "\n";
...
// Functions that hook into thematic_header()
		...
		function thematic_blogtitle() { ?>
		    		<a href="/"><div id="blog-title"></div></a>
		<?php }
		...
		function thematic_brandingclose() { ?>
					<div id='search_site'>
						<div class='search_show'>
							<a href='#' onclick='jQuery("#search_site .search_form").toggle();jQuery("#search_site .search_form #search_site_text").focus();return false;'>
								Search
							</a>
						</div>
						<div class='search_form' style='display: none'>
							<form name='search' action='/' method='get'>
								<input type='text' id='search_site_text' name='s' onfocus="if (this.value == 'Search...') {this.value = '';}" onblur="if (this.value == '') {this.value = 'Search...';}" value="<?php echo $_GET['s'] ? $_GET['s'] : 'Search...' ?>" />
		...
		function thematic_access() { ?>
		        ...
		        <?php thematic_social_icons(); ?>
		      </div><!-- #access -->
		<?php }
		add_action('thematic_header','thematic_access',9);
		
		function thematic_social_icons(){
		  $icons = array(
		    array("url" => "/feed", "title" => "Subscribe to my RSS Feed", "image" => "icon-rss.png", "hover-image" => "icon-rss-over.png"),
		    array("url" => "http://twitter.com/coffeencoke", "title" => "Follow me on Twitter", "image" => "icon-twitter.png", "hover-image" => "icon-twitter-over.png"),
		    array("url" => "http://flickr.com/matt_simpson", "title" => "View my Photos", "image" => "icon-flickr.png", "hover-image" => "icon-flickr-over.png"),
		    array("url" => "http://www.last.fm/user/msimpson3", "title" => "Check out my Music", "image" => "icon-lastfm.png", "hover-image" => "icon-lastfm-over.png"),
		    array("url" => "http://www.facebook.com/coffeencoke", "title" => "Find me on Facebook", "image" => "icon-facebook.png", "hover-image" => "icon-facebook-over.png")
		  );
		  ?>
            <div id='social_icon_bar'>
              <ul>
                ...
                <li>
                  <a href='<?php echo $icon['url']; ?>' target='_blank' title='<?php echo $icon['title']; ?>'>
                    <img class='rollover' src='/wp-content/themes/roots/images/<?php echo $icon['image']; ?>' hover='/wp-content/themes/roots/images/<?php echo $icon['hover-image']; ?>' />
                  </a>
                </li>
                <?php } ?>
              </ul>
