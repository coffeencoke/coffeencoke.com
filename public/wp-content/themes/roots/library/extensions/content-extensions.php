...
function thematic_page_title() {
		...
		if (is_attachment()) {
				...
				$content .= '<div id="page_title">';
				$content .= '<div id="page_title_home">';
				$content .= '</div>';
				$content .= '</div>';
		}
		$content .= "\n";
...
function thematic_postheader() {
    ...
    // 
    // // Create the date stamp box with month and date.
    // $postdate  = "<div class='date'>";
    // $postdate .= "<span class='month'>" . get_the_time('M') . " </span>";
    // $postdate .= "<span class='day'>" . get_the_time('j') . "</span>";
    // $postdate .= "</div>";
    
    if (is_single() || is_page()) {
        $posttitle = '<h1 class="post-title">' . get_the_title() . "</h1>\n";
    } elseif (is_404()) {    
        $posttitle = '<h1 class="post-title">' . __('Not Found', 'thematic') . "</h1>\n";
    } else {
        $posttitle = '<h1 class="post-title"><a href="';
    ...
    //     $postmeta = '<div class="meta">';
    // $postmeta .= '<h2>By <a href="' . get_author_link(false, $authordata->ID, $authordata->user_nicename);
    // $postmeta .= '" title="' . __('View all posts by ', 'thematic') . get_the_author() . '"> ';
    //     $postmeta .= get_the_author() . '</a></h2>' . // get_the_time();
    // 
    // // Display the post categories  
    // $postcategory = '';
    // if (is_single()) {
    //     $postcategory .= __('<br />Posted in ', 'thematic') . get_the_category_list(', ');
    // } elseif ( is_category() && $cats_meow = thematic_cats_meow(', ') ) { /* Returns categories other than the one queried */
    //     $postcategory .= __('<br />Also posted in ', 'thematic') . $cats_meow;
    // } else {
    //     $postcategory .= __('<br />Posted in ', 'thematic') . get_the_category_list(', ');
    // }
    ...
    //     
    // //$postmeta .= $postcategory . $posttags;
    // 
    ...
    //             $postcomments = '<a href="' . get_permalink() . '#respond" title="' . __('Comment on ', 'thematic') . the_title_attribute('echo=0') . '">';
    ...
    //             $postcomments = '<a href="' . get_permalink() . '#respond" title="' . __('Comment on ', 'thematic') . the_title_attribute('echo=0') . '">';
    //             $postcomments .= __('Comment, be the first!', 'thematic') . '</a>';
    //         }
    //     } else {
    ...
    // 
    //     $postmeta = apply_filters('thematic_postheader_postmeta',$postmeta . $postcomments);     
    //     // Display edit link
    //     if (current_user_can('edit_posts')) {
    //         $postmeta .= '<br />' . $posteditlink;
    //     }
    // 
    // $end .= "</div>\n";
    ...
    // } else {
    //     $postheader = $posttitle . $postdate  . $postmeta . $end;    
    // }
    
    echo apply_filters( 'thematic_postheader', $postheader ); // Filter to override default post header
} // end thematic_postheader

function thematic_post_meta(){
    global $id, $post, $authordata;
	if(in_array(11, wp_get_post_categories($post->ID))){
		return thematic_portfolio_post_meta();
	}

    // Create time text
    $posttime  = "<span class='time'>at " . get_the_time('g:i a') . " by ";
    $posttime .= "<a href='" . get_author_link(false, $authordata->ID, $authordata->user_nicename) . "' ";
	$posttime .= '" title="' . __('View all posts by ', 'thematic') . get_the_author() . '"> ';
    $posttime .= get_the_author() . '</a></span>' . // get_the_time();

    // Display the post categories  
   	$postcategory = "<br /><span class='categories'>".__('Categorized in ', 'thematic') . get_the_category_list(', ') . "</span>";
    $postcategory = apply_filters('thematic_postfooter_postcategory',$postcategory); 
    
    // Display the tags
	$posttags = "<br /><span class='tags'>Tagged with " . get_the_tag_list("$tagtext ",', ') . "</span>";
    $posttags = apply_filters('thematic_postfooter_posttags',$posttags); 
    
    // Display comments link
    if (comments_open()) {
		$postcomments = "<br /><span class='comments-link'>";
        $postcommentnumber = get_comments_number();
        if ($postcommentnumber > '1') {
            $postcomments .= '<a href="' . get_permalink() . '#respond" title="' . __('Comment on ', 'thematic') . the_title_attribute('echo=0') . '">';
            $postcomments .= get_comments_number() . __(' Comments', 'thematic') . '</a>';
            ...
            $postcomments .= '<a href="' . get_permalink() . '#respond" title="' . __('Comment on ', 'thematic') . the_title_attribute('echo=0') . '">';
            $postcomments .= __("Comment, you're the first!", 'thematic') . '</a>';
        }
		$postcomments .= "</span>";
    } else {
        $postcomments = '<span class="comments-link comments-closed-link">' . __('Comments closed', 'thematic') .'</span>';
    ...
    // Create the date stamp box with month and date.
    $postdate  = "<div class='date'>";
    $postdate .= "<div class='month'>" . get_the_time('M') . "</div>";
    $postdate .= "<div class='day'>" . get_the_time('j') . "</div>";
    $postdate .= "<div class='year'>" . get_the_time('Y') . "</div>";
    $postdate .= "</div>";

    $open = "<div class='postmeta'>";
	$close = "</div>";

    $postmeta = apply_filters('thematic_postheader_postmeta', $open . $posttime . $postcategories . $posttags . $postcomments . $close . $postdate);
    
    
    echo apply_filters( 'thematic_postmeta', $postmeta ); // Filter to override default post header
}

function thematic_portfolio_post_meta(){
    global $id, $post, $authordata;	
    // Create time text
    $posttime  = "<span class='time'>at " . get_the_time('g:i a') . " by ";
    $posttime .= "<a href='" . get_author_link(false, $authordata->ID, $authordata->user_nicename) . "' ";
...
function thematic_content() {
	...
	$post = '<div class="content white">' . $post . '</div>';
