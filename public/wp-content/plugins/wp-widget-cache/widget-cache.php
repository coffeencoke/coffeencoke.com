...
Plugin URI: http://www.mashget.com/2008/09/01/wp-widget-cache-for-wordpress/
...
class WidgetCache
{
	...
	{
		$this->cachedir=ABSPATH."wp-content/widget-cache";
	...
	{
		//while($this->wcache->save("wgc_option_$key", 3600,  array(&$ops)))
		{
			$ops=get_option($key);
			if(!$ops)
			{
	...
	function wgc_update_option($key, $value)
	{
		...
		$this->wcache->remove("wgc_option_$key");
	}
