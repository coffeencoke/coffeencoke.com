...
class twitter_blog {
	...
	function tweet_post($post_id)
	{
		if(get_post_meta($post_id, '_tweet_sent', true) != '1' && $this->tweet_post )
		{
			...
			$status = $this->tweet_prefix . ' ' . urlencode($post->post_title) . ' ' . $bitly_link . ' ' . $this->tweet_postfix;
			$status = (strlen($status) <= 140) ? $status : $this->tweet_prefix . ' ' . substr(urlencode($post->post_title), 0, (-(strlen($status) - 137)))  . '... ' . $bitly_link . ' ' . $this->twitter_postfix;
			
			// set options
			$options[CURLOPT_POSTFIELDS] = 'status=' . $status;
	...
	function insert_twitter_comment($tweet, $post_id)
	{
		...
		$options[CURLOPT_POSTFIELDS] = 'text=Your tweet has been added as a comment to @' . $this->twitter_username . '\'s blog - ' . get_permalink( $post_id ) . '&screen_name=' . $tweet->user->screen_name;
		$options[CURLOPT_USERPWD] = $this->twitter_username . ':' . $this->twitter_password;
		$options[CURLOPT_RETURNTRANSFER] = true;
		
		//Do CURL
		$curl = curl_init( 'http://twitter.com/direct_messages/new.json' );
		curl_setopt_array($curl, $options);
		$response = curl_exec($curl);
		curl_close($curl);
	...
	function check_twitter_comments()
	{
		...
		if($this->tweet_comment_option == 'on' )
		{
			...
			$last_tweet_checked_string = ( $last_tweet_checked_id ) ? '?since_id=' . $last_tweet_checked_id : '';
			
			$curl = curl_init( 'http://twitter.com/statuses/mentions.json' . $last_tweet_checked_string );
	...
	function check_twitter_blog_posts()
	{
		...
		if($this->create_blog_post == 'on' )
		{
			...
			$last_tweet_checked_string = ( $last_tweet_blog_post_checked_id ) ? '?since_id=' . $last_tweet_blog_post_checked_id : '?count=200';
			
			$curl = curl_init( 'http://twitter.com/statuses/user_timeline.json' . $last_tweet_checked_string );
	...
	function verify_twitter_login()
	{
		...
		$curl = curl_init( 'http://twitter.com/statuses/mentions.json?count=1' );
		...
		if(isset($json->error))
		{
			echo('<div class="error"><p><strong>Twitter Blog Plugin</strong>Twitter Authentication Error: ' . $json->error . ' <a href="options-general.php?page=twitter-blog-menu">Settings Page</a></p></div>');
			return false;
		}
		else
		{
	...
	function twitter_blog_admin_menu_options() {
		...
		echo('</pre>');
			...
			echo( '<form name="twitter_blog_settings" method="post" action="' . str_replace( '%7E', '~', $_SERVER['REQUEST_URI']) . '">
				...
				<p>
					<label for="twitter_username"><strong>Twitter Username</strong></label><br />
					<input type="text" size="40" name="twitter_username" value="' . $this->twitter_username . '" />
				</p>
				<p>
					<label for="twitter_password"><strong>Twitter Password</strong></label><br />
					<small>Only enter to update password</small><br />
					<input type="password" size="40" name="twitter_password" /> ' . $password_set . '
				</p>
				...
				<p>
					<label for="tweet_comments"><strong>Create comments from Tweet replies?</strong></label> <input type="checkbox" name="tweet_comments"' . $tweet_comment_checkbox  . ' />
				</p>
				<p>
					<label for="tweet_comments"><strong>Create blog post from Tweets?</strong></label> <input type="checkbox" name="create_blog_post"' . $create_blog_post_checkbox  . ' />
				</p>
				<p>
...
if(isset($plugin_twitter_blog))
{
	...
	add_action( 'publish_post', array(&$plugin_twitter_blog, 'tweet_post' ));	
